...
def rms_skew_assign_weights(rms_skew,
                            rms_sd,
                             all_heatmaps,
                             all_xedges,
                             all_yedges):
    ...
    #print("RMS SKEW ASSIGN WEIGHTS = ", rms_skew)
    #print("RMS SD ASSIGN WEIGHTS =", rms_sd)

    #print("Assign weights heatmap = ", all_heatmaps)
    #print("Assign weights xedges = ", all_xedges)
    #print("Assign weights yedges = ", all_yedges)
    ...
    for classifier_index in range(len(all_heatmaps)):
        ...
        for x_edge in range(len(x_category)-1):
            ...
            if rms_skew >= low_boundary and rms_skew <= high_boundary:
                x_heatmap_index = x_edge
                pass
            elif rms_skew < low_boundary:
        ...
        for y_edge in range(len(y_category)-1):
            ...
            if rms_sd >= low_boundary and rms_sd <= high_boundary:
                y_heatmap_index = y_edge
                pass
            elif rms_sd < low_boundary:
                pass
            else:
                pass
