...
def assign_weights(y_variable,
    ...
    for classifier_index in range(len(all_heatmaps)):
        ...
        for x_edge in range(len(x_category)-1):
            ...
            if x_variable >= low_boundary and x_variable <= high_boundary:
                x_heatmap_index = x_edge
                pass
            elif x_variable < low_boundary:
        ...
        for y_edge in range(len(y_category)-1):
            ...
            if y_variable >= low_boundary and y_variable <= high_boundary:
                y_heatmap_index = y_edge
                pass
            elif y_variable < low_boundary:
                pass
            else:
                pass
