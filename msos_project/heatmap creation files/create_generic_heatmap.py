...
def Single_File_Classification(input_path,
    ...
    if test_method == True:
        ...
        effective_duration_cutoff = 0.2  # percentage of maximum to consider the "sustain" part of the signal
        ...
        #pyplot.scatter(first_start_value_index, first_start_rms_value, label='Attack time start')
        #pyplot.scatter(first_cutoff_value_index, first_cutoff_rms_value, label='Attack time end')
        #pyplot.scatter(max_rms_value_location, max_rms_value, label='Max value')
        pyplot.plot(y_axis_val, label='Effective duration cutoff energy')
        pyplot.xlabel("Time (bins)")
        pyplot.ylabel("RMS energy")
        pyplot.legend()
        pyplot.show()
    ...
    feature_1_name = "Effective Duration"
    feature_2_name = "Log attack time"
