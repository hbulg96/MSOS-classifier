...
def Single_File_Classification(input_path,
    ...
    category_weights = rms_skew_weights
    print("RMS Skew/RMS sd category weights = ", rms_skew_weights)


    category_weights = sc_fm_weights
    print("Contrast feature-max/noise-min category weights = ", sc_fm_weights)

    category_weights = sc_mean_cov_weights
    print("Contrast feature band mean/CoV category weights = ", sc_mean_cov_weights)

    category_weights = no_peak_cf_weights

    category_weights = effective_duration_weights
    """

    category_weights = CombineWeights(zcr_category_weights,
                                      sc_category_weights,
                                      rms_skew_weights,
                                      sc_fm_weights,
                                      sc_mean_cov_weights,
                                      no_peak_cf_weights,
                                      effective_duration_weights)
...
def CombineWeights(input_weights1,
                   input_weights2,
                   input_weights3,
                   input_weights4,
                   input_weights5,
    ...
    """
    # weighted averaging method (mixes the two overall accuracies, not good!)

    input_prediction_accuracy1 = [0.553, 0.336, 0.523, 0.460, 0.383]
    # category prediction accuracy for zero crossing weight method
    input_prediction_accuracy2 = [0.373, 0.556, 0.34, 0.436, 0.743]
    # category prediction accuracy for spectral centroid method

    zcr_prediction_ratios = []
    sc_prediction_ratios = []
    
    for n in range(len(input_prediction_accuracy1)):
        accuracy1 = input_prediction_accuracy1[n]
        accuracy2 = input_prediction_accuracy2[n]

        total_of_accuracies = accuracy1 + accuracy2

        prediction_ratio1 = (accuracy1/total_of_accuracies)  #find ratio wrt 1 of prediction accuracy for first layer
        prediction_ratio2 = (accuracy2/total_of_accuracies) # find ratio wrt 1 of prediction accuracy for second layer
        
        zcr_prediction_ratios.append(prediction_ratio1) # store prediction ratios for each layer
        sc_prediction_ratios.append(prediction_ratio2) #
        pass
    """
        

    """
    # mixed classification method, completely useless
    for x in range(len(input_weights1)):

        value1 = input_weights1[x]
        value2 = input_weights2[x]

        prediction_accuracy1 = input_prediction_accuracy1[x]
        prediction_accuracy2 = input_prediction_accuracy2[x]

        print("value1 = ", value1)
        print("value2 = ", value2)
        print("Prediction accuracy for ZCR = ", prediction_accuracy1)
        print("Prediction accuracy for SC = ", prediction_accuracy2)

        if prediction_accuracy1 >= prediction_accuracy2:
            output_weights.append(value1)
            pass

        elif prediction_accuracy1 < prediction_accuracy2:
            output_weights.append(value2)
            pass

        else:
            print("Error in detecting prediction accuracy in CombineWeights()")
            pass
    """

    # simple averaging method

    for n in range(len(input_weights1)):

        output_weight = (input_weights1[n]
                         + input_weights2[n]
                         + input_weights3[n]
                         + input_weights4[n]
                         + input_weights5[n]
...
def CheckCategoryFromWeights(input_category_weights):
    ...
    if input_category_weights == [0, 0, 0, 0, 0]:
        category_index = numpy.random.randint(0,4)
