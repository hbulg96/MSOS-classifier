...
def Single_File_Classification(input_path,
    ...
    return(rms_in_time, window_values)
    ...
    for n in range(5):
        ...
        for x in range(300):
            ...
            if exact_file_index in previously_tested_file_indices:
                ...
                rms_in_time, window_values = Single_File_Classification(input_path,
                                                               file_category,
                                                               rhythm_method,
                                                               spectral_centroid_method,
                                                                zero_crossing_rate_method,
                ...
                ###
                category_rms.append(rms_in_time)
                category_window_values.append(window_values)
...
##                print("Number of predictions correct = ", prediction_correct, " of ", n_index)
        ...
        all_categories_rms.append(category_rms)
        """
        print("All category rms = ", category_rms)
        print("category window values = ", category_window_values)

        for individual_rms_list in category_rms:
            matplotlib.pyplot.scatter(window_values, individual_rms_list)
            matplotlib.pyplot.plot(window_values, individual_rms_list)
            pass
        ...
        output_yedges = []
        x = 0
        range_val = [[0, 8000], [0, 20000]]
        bins_val = [9, 26]
        for rms_series in category_rms:
            y_val = []
            for val in range(len(rms_series)):
                val = val*1000
                y_val.append(val)
            x_val = rms_series
            heatmap, xedges, yedges = numpy.histogram2d(y_val, x_val,bins=bins_val, range=range_val)
        ...
        pyplot.show()
        print("Output heatmap = ", output_heatmap)
        

##        print("Category rms = ", category_rms)
##        print("Window values = ", window_values)
##        scaled_window_values = []
##
##        #for rms_values in category_rms:
##        heatmap, xedges, yedges = numpy.histogramdd(category_rms, bins=20)
##        extent = [xedges[0], xedges[-1], yedges[0], yedges[-1]]
##
##        matplotlib.pyplot.clf()
